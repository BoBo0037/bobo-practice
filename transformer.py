from utils.helper import set_device, download, load, check_and_make_folder
import math
import tiktoken
import torch
...
print("Number of tokens in tokenized_text:", len(tokenized_text))
...
class Attention(nn.Module):
    def __init__(self, head_size: int):
        ...
        self.register_buffer('tril', torch.tril(torch.ones((self.context_length, self.context_length))))  # Lower triangular mask
...
def get_batch(split: str):
    ...
    y = torch.stack([data[idx + 1 : idx + 1 + context_length] for idx in idxs]).to(device) # 右移一位(答案), 用做训练 
    return x, y
...
for step in range(max_iters):
    ...
    optimizer.zero_grad(set_to_none=True) # 梯度重置为None
